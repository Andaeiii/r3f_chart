...
function App() {
  ...
  const getRandomHexColor = () =>
    `#${Math.floor(Math.random() * 16777215)
      ...
      .padStart(6, "0")}`;
  const sdata = [
    { label: "Lawyers", value: 10, min: -50, max: 100, color: "#f00" },
    {
      label: "Soldiers",
      value: -40,
      min: -50,
  ...
  const [pstiv, setPstiv] = useState(false);
  ...
  const readBarValues = (dt) => {
    // console.log(dt, 'dtx');
    setData(dt);
    allPositive = sdata.every((item) => item.value > 0);
  };

  useEffect(() => {
    setPstiv(allPositive);
    console.log(pstiv);
  }, [data]);
    ...
    data && (
      <div className="parent">
        ...
        <div className="sideXbar">
          ...
          <hr />
          <SliderGroup dataFromSlider={readBarValues} data={data} />
        </div>
      </div>
    )
  );
